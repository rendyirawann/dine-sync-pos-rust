...
- [x] **Master data CRUD: Kategori + Menu + Meja** SELESAI 2026-06-10 (`master.rs`, `master/{categories,menus,tables}.html`): list + tambah/edit (modal) + hapus dgn **guard relasi** (kategori berisi menu / menu dipakai order / meja punya order → ditolak), gate `view_data_master`, slug auto. Terverifikasi. *Sisa master:* Promo, Supplier, Ingredient.
